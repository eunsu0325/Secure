...
"""
☘️ FAR 타겟 기반 임계치 계산 및 스무딩 (EER 모드 제거)
...
import numpy as np
# ☘️ from sklearn.metrics import roc_curve  # 삭제 — EER 제거로 불필요
# ☘️ from scipy.interpolate import interp1d  # 삭제 — EER 제거로 불필요
# ☘️ from scipy.optimize import brentq       # 삭제 — EER 제거로 불필요
...
class ThresholdCalibrator:
    """
    ☘️ 전역 임계치(τ_s) 캘리브레이터 — FAR 모드 단독 (EER 제거)
    - τ = quantile(unknown_dev_scores, 1 - target_FPIR)
    - 코사인 유사도 기반(기본)
    - EMA 스무딩 + 변화폭 제한
    - (옵션) 마진 자동 조정
    ...
    # ☘️ compute_eer_threshold 삭제 — EER 모드 제거
    # ☘️ def compute_eer_threshold(self, genuine, impostor):
    # ☘️     """EER 임계치 계산 — FAR 모드로 대체됨"""
    # ☘️     ...  (roc_curve, brentq, interp1d 사용 — 의존성 제거)
    ...
    def calibrate(self, genuine_scores: np.ndarray, impostor_scores: np.ndarray,
        ...
        Returns:
            캘리브레이션 결과 딕셔너리
        """
        # ☘️ FAR 모드 단독 사용 (EER 분기 제거)
        tau_new, achieved_metric = self.compute_far_threshold(impostor_scores)
        metric_name = "far"
        # ☘️ if self.threshold_mode == "far": ... else: compute_eer_threshold(...)  # 삭제
...
if __name__ == "__main__":
    ...
    # ☘️ EER 모드 비교 테스트 제거
    # ☘️ calibrator_eer = ThresholdCalibrator(threshold_mode="eer", ...)  # 삭제
    # ☘️ result_eer = calibrator_eer.calibrate(genuine, impostor, ...)   # 삭제

    print("\n" + "="*50)
    print("☘️ FAR Mode Results (EER 모드 제거됨)")
    print("-" * 50)
